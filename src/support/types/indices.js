
export const topLevelPages = [ "about", "english", "recipes", "portfolio", "coming-soon" ];

export const mainCategories = [ "about", "english", "recipes", "portfolio" ];

//About subcategories
export const aboutSubcategories = [ "authors" ];
export const aboutTags = [ "alex" ];

//English subcategories
export const englishSubcategories = [ "tenses", "conditionals", "questions", "tricky-words" ];
export const englishTags = [ "past", "present", "future", "informal", "formal" ];

//Portfolio subcategories
export const portfolioSubcategories = [ "photography", "writing" ];
//fiction relates to fictional stories/poems I write, stories relate to any story fact or fiction I write. Reviews relates to other author stories.
export const portfolioTags = [ "diet", "exercise", "fiction", "family-life", "health", "information", "malta", "poems", "reviews", "stories" ];

///Recipes subcategories
export const recipeSubcategories = [ "by-diet", "by-ingredient", "by-course", "by_option" ];
export const byDiets = [ "carnivore", "low-calorie", "low-carb", "paleo", "vegetarian" ];
export const byOptions = [ "bread", "comfort-food", "curry", "everyday", "family", "meal-in-one", "pies", "simple", "take-out" ];
export const byIngredients = [ "beef-and-lamb", "pasta-grains-and-alternatives", "chicken-turkey-and-pork", "fish-and-seafood", "fruits-nuts-and-vegetables", "dairy-eggs-and-mushrooms" ];
export const byCourses = [ "breakfast-and-lunch", "condiments", "main-dish", "side-dish", "salads-and-soups", "snacks-and-appetisers", "desserts" ];
export const recipeTags = [ "atkins", "egg-free", "gluten-free", "grain-free", "keto", "lactose-free", "indulgence", "nut-free", "strict-paleo", "slow-cooked", "vinaigrette" ];

//Types
//post refers to anything which is worth being on the home page
export const types = [ "post", "profile", "recipe", "article", "poem", "photograph", "lesson" ];

//Changing Posts
///A favourite recipes list - this will be manually altered occasionally!
export const favouriteRecipes = [
  "keto-prawn-cocktail-with-marie-rose-sauce",
  "peach-meringue-roulade",
  "beefburgers-with-a-tangy-sauce",
  "creamy-parsnip-and-leek",
  "banoffee-pie",
  "butternut-squash-and-carrot-soup"
];
//Used in the about page in the aside - this will be manually altered occasionally!
export const recommendedPosts = [
  "blackened-shrimp",
  "my-pregnancy-story",
  "avocado-filled-with-tuna-mayo",
  "calculating-a-keto-day",
  "almond-flour-skillet-bread",
  "the-characters-of-david-gemmell"
];
//Used in the homepage in the carousel - this will be manually altered occasionally!
export const recipesOfMonday = [
  "bacon-and-eggs",
  "shrimp-salad",
  "cajun-chicken",
  "parsnip-chips",
  "mixed-berries-on-greek-yoghurt",
];
export const recipesOfTuesday = [
  "easy-sausage-rolls",
  "mushroom-soup",
  "crusty-bread",
  "blackened-shrimp",
  "tomato-and-cucumber-salad",
  "roasted-broccoli",
  "mango-pudding-with-coconut-cream-topping",
];
export const recipesOfWednesday = [
  "almond-flour-sausage-rolls",
  "corned-beef-and-egg-hash",
  "pork-chops-with-apple-cinnamon-sauce",
  "mashed-cauliflower",
  "roasted-carrots"
];
export const recipesOfThursday = [
  "avocado-filled-with-tuna-mayo",
  "coriander-chicken-salad-with-lime-mayonnaise",
  "beef-cubes-in-a-bowl",
  "banoffee-pie",
];
export const recipesOfFriday = [
  "mixed-berries-on-greek-yoghurt",
  "low-cal-mild-turkey-korma",
  "margherita-pizza",
  "sweet-potato-fries",
];
export const recipesOfSaturday = [
  "goats-and-cheddar-cheese-omelette",
  "keto-prawn-cocktail-with-marie-sauce",
  "chicken-basque",
  "mashed-cauliflower",
  "peach-meringue-roulade",
];
export const recipesOfSunday = [
  "keto-brunch",
  "roast-chicken",
  "roasted-autumn-root-vegetables",
  "avocado-walnut-and-broccoli-salad",
  "treacle-sponge",
];



 