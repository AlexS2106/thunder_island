...
import { doubleArrowRight, doubleArrowLeft } from "../../../../support/functions/iconFunctions";
...
const Pagination = ( { pageInfo } ) => {
  ...
  const paginationList = paginationArray.map( ( el, index ) => {
    ...
    if ( /*is a middle page*/ currentPage > 3 && currentPage <= pageCount - 2 ) {
      if ( i === prevArrow ) return <li key={ uuid() } ><Link to={ `/${ prevArrow }` } className={ arrow } > { doubleArrowLeft() }</Link></li>;
      if ( i === currentPage || i === nextPageEl || i === prevPageEl ) return el;
      if ( i === nextArrow ) return <li key={ uuid() } ><Link to={ `/${ nextArrow }` } className={ arrow } > { doubleArrowRight() }</Link></li>;
    } else if ( currentPage /*is third page*/ === 3 ) {
      if ( i === prevArrow ) return <li key={ uuid() } ><Link to={ `/` } className={ arrow } > { doubleArrowLeft() }</Link></li>;
      if ( i === currentPage || i === nextPageEl || i === prevPageEl ) return el;
      if ( i === nextArrow ) return <li key={ uuid() } ><Link to={ `/${ nextArrow }` } className={ arrow } > { doubleArrowRight() }</Link></li>;
      ...
      if ( i === currentPage || i === nextPageEl || i === nextPageEl + 1 ) return el;
