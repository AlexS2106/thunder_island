...
import { singleArrowLeft, singleArrowRight } from "../../../support/functions/iconFunctions";
...
//Generates the carousel posts
  ...
  const clickHandler = ( value ) => { 
    if ( value === "increment" ) {
      move >= (carouselData * multiplyBy) ? setMove( carouselData * multiplyBy ) : setMove(move + multiplyBy)
  ...
  return (
    <div className={ carousel } >
      <div className={ `flexRow ${ centered }` }>
        ...
        <h3>{ title }</h3>
        {
          move < ( ( carouselData.length * multiplyBy ) / ( carouselData.length / 2 ) ) ?
            <button
              onClick={ () => clickHandler( "increment" ) }>
              <span className={ colorPadding }>
                { singleArrowRight() }
              </span>
            </button> : null
        }
      ...
      <div
        ...
        style={ { width: `${ carouselData.length * multiplyBy }vw`, transform: `translateX( -${ move }vw )` } } >
        { generateCarousel }
      </div>
    </div>
  );
