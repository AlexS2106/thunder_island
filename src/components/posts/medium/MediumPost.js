...
import { makeLink, listSubcategories, reduceSentenceLength } from "../../../support/functions/utility";
...
const MediumPost = ( { postData, ...props } ) => {
  ...
  const { title, slug, mainCategories, author, portraitImage, alt, photographer, description } = frontmatter;
  //article -> Button
  ...
  const generatedExcerpt = reduceSentenceLength(description, props.excerptLength || 100);
  ...
  const mainCats = mainCategories.map( ( category ) => {
    return (
      <h4 key={ uuid() } className="textCenter shadowText" >{ category.name }</h4>
    );
  } );
  ...
  return (
    <article className={ `bgLight ${post}` }>
      <div>
        <h3 className="textCenter shadowText">
          { title }
        </h3>
        { mainCats }
        { props.showSubcategories && subCats }
        { props.showAuthor &&
          <address rel="author" className="textCenter shadowText" >
            by { author }
          </address> }
      </div>
